...
def graph_based_app(opt):
    print(opt)
    from n3ml.layer import IF1d, LIF1d, CUBA_LIF1d
    # generate time_line
    s_time = np.arange(0, opt.sim_time, 1)

    # Initilize IF1d population
    if1d_pop1 = CUBA_LIF1d(neurons=opt.n_neurons, threshold=opt.v_th, time_interval = opt.sim_time, tau_rc=opt.decay, q=opt.q)
