...
def train(loader, model, encoder, optimizer, criterion, opt):
    num_images = 0
    total_loss = 0.0
    num_corrects = 0
...
def app(opt):
    ...
    for epoch in range(opt.num_epochs):
        tr_loss, tr_acc = train(train_loader, model, encoder, optimizer, criterion, opt)
        print("epoch: {} - loss: {} - accuracy: {}".format(epoch, tr_loss, tr_acc))

        val_loss, val_acc = validate(val_loader, model, encoder, criterion, opt)
        print("In test, loss: {} - accuracy: {}".format(val_loss, val_acc))
