...
class Diehl2015(nn.Module):
    ...
    def forward(self, x, find_threshold = False, threshold:List[float]=None):
        x = self.conv1(x)
        x = self.conv1_relu(x)  # ReLU
        if find_threshold == True and threshold:
            threshold[0] = max(threshold[0], x.max())
        x = self.conv1_drop(x)
        x = self.pool1(x)
        x = self.conv2(x)
        x = self.conv2_relu(x)  # ReLU
        if find_threshold == True and threshold:
            threshold[1] = max(threshold[1], x.max())
        x = self.conv2_drop(x)
        x = self.pool2(x)
        x = x.view(x.size(0), -1)
        x = self.fc1(x)
        if find_threshold == True and threshold:
            threshold[2] = max(threshold[2], x.max())
        return x
...
class SNN_Huynh2021(nn.Module):
    def __init__(self, batch_size: int):
        ...
        self.conv1 = nn.Conv2d(in_channels=1, out_channels=12, kernel_size=(5, 5), stride=(1, 1), padding=0, bias=False)
        self.conv1_if = SoftIF2D_channel_thresholds(batch_size=batch_size,num_channels=12,height=24, width=24,reset=0.0)
        self.pool1 = nn.AvgPool2d(2)
        self.conv2 = nn.Conv2d(in_channels=12, out_channels=64, kernel_size=(5, 5), stride=(1, 1), padding=0,
                               bias=False)
        self.conv2_if = SoftIF2D_channel_thresholds(batch_size=batch_size, num_channels=64, height=8, width=8,reset=0.0)
        self.pool2 = nn.AvgPool2d(2)
        self.fc1 = nn.Linear(in_features=1024, out_features=10, bias=False)
        self.fc1_if = IF1d_NoSpike(batch_size=batch_size,num_features=10)
    ...
    def init_neuron_models(self):
        for m in self.named_children():
            if isinstance(m[1], IF1d_NoSpike) or isinstance(m[1], SoftIF2D_channel_thresholds):
                m[1].init_vars()
    ...
    def forward(self, images: torch.Tensor, inference = False, ):
...
class SNN_Diehl2015(nn.Module):
    def __init__(self, batch_size: int, threshold: List[float] = None):
        super(SNN_Diehl2015,  self).__init__()
        self.batch_size = batch_size
        ...
        self.conv1 = nn.Conv2d(in_channels=1, out_channels=12, kernel_size=(5, 5), stride=(1, 1), padding=0, bias=False)
        self.conv1_if = IF2d(batch_size=self.batch_size,planes=12,height=24,width=24,threshold=threshold[0],reset=0.0)
        self.pool1 = nn.AvgPool2d(2)
        self.conv2 = nn.Conv2d(in_channels=12, out_channels=64, kernel_size=(5, 5), stride=(1, 1), padding=0,bias=False)
        self.conv2_if = IF2d(batch_size=self.batch_size,planes=64,height=8,width=8,threshold=threshold[1],reset=0.0)
        self.pool2 = nn.AvgPool2d(2)
        self.fc1 = nn.Linear(in_features=1024, out_features=10, bias=False)
